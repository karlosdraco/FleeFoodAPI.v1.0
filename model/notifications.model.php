        ...
        public function fetchNotification($loggedUserId){
            ...
            if($statement->execute()){
                if($statement->rowCount() > 0){
                   ...
                   $statement = $this->conn->query("SELECT notifications.*,users.firstname, users.lastname, users.profile_image
                    FROM notifications RIGHT JOIN users ON notifications.agent_id=users.id
                    RIGHT JOIN food_post ON notifications.food_id=food_post.id WHERE notifications.user_id=:uid ORDER BY notifications.notif_date DESC");
